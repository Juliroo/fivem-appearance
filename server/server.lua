...
RegisterServerEvent("fivem-appearance:server:chargeCustomer", function(shopType)
    ...
    if Player.Functions.RemoveMoney('cash', money) then
        TriggerClientEvent("QBCore:Notify", src, "Pagaste "..money.."$", "success")
    else
    ...
    if model and components and props then
        MySQL.Async.insert(
            ...
            {Player.PlayerData.citizenid, name, model, json.encode(components), json.encode(props)}, function(id)
                ...
                TriggerClientEvent('QBCore:Notify', src, "Guardaste el atuendo con el nombre "..name.."", "success")
            end)
    end
end)
    ...
    QBCore.Commands.Add('pedmenu', 'Open Ped Menu', {{
        ...
        if playerId then
            ...
            if Player then
                ...
                TriggerClientEvent('QBCore:Notify', src, "Jugador no conectado", 'error')
                return
            end
        end
...
QBCore.Commands.Add('fixpj', 'fixpj', {}, false, function(source, _)
