...
RegisterNetEvent('fivem-appearance:client:saveOutfit', function()
    local keyboard = exports['qb-input']:ShowInput({
        header = "Nombra tu atuendo",
        submitText = "Guardar",
        inputs = {{
            text = "Nombre del atuendo",
            name = "input",
            type = "text",
            isRequired = true
        }}
    ...
    if keyboard ~= nil then
        ...
        QBCore.Functions.TriggerCallback("fivem-appearance:server:getOutfits", function(outfits)
            ...
            if outfitExists then
                QBCore.Functions.Notify("El atuendo con este nombre ya existe.", "error")
                return
            end
...
RegisterNetEvent("fivem-appearance:client:OutfitManagementMenu", function(args)
    ...
    --     {
    --         header = 'Administrar Atuendo',
    --         isMenuHeader = true,
    --     },

    --     {
    --         header = "Cambiar atuendo",
    --         params = {
    --             event = "copas",
    --         }
    --     },  
...
RegisterNetEvent("fivem-appearance:client:SaveManagementOutfit", function(mType)
    ...
    local saveDialog = exports['qb-input']:ShowInput({
        header = "Detalles del equipo de gestión",
        submitText = "Guardar atuendo",
        inputs = {
            {
                text = "Nombre del atuendo",
                name = "outfitName",
    ...
    QBCore.Functions.TriggerCallback('fivem-appearance:server:getManagementOutfits', function(result)
        ...
        for i = 1, #result, 1 do
            outfitMenu[#outfitMenu + 1] = {
                header = 'Borrar "' .. result[i].name .. '"',
                txt = "Modelo: " .. result[i].model .. " - Género: " .. result[i].gender,
                params = {
...
function OpenMenu(isPedMenu, backEvent, menuType, menuData)
    ...
    if menuType == "default" then
        local header = "Comprar ropa"
        if isPedMenu then
            header = "Cambiarse de ropa"
        end
        menuItems[#menuItems + 1] = {
            header = "Tiendas de ropa",
    ...
    if pedModel ~= data.model then
        QBCore.Functions.TriggerCallback("fivem-appearance:server:getAppearance", function(appearance)
            if appearance then
                ...
                QBCore.Functions.Notify("Algo salió mal. El atuendo al que intentas cambiarte no tiene una apariencia básica..","error")
                failed = true
            end
        end, data.model)
    else
...
RegisterNetEvent("fivem-appearance:client:deleteOutfitMenu", function(data)
    QBCore.Functions.TriggerCallback('fivem-appearance:server:getOutfits', function(result)
        ...
        for i = 1, #result, 1 do
            outfitMenu[#outfitMenu + 1] = {
                header = 'Eliminar ' .. result[i].outfitname .. '',
                params = {
...
RegisterNetEvent("fivem-appearance:client:DeleteManagementOutfit", function(id)
    ...
    QBCore.Functions.Notify('Atuendo eliminado', 'info')
end)
...
local function InCooldown()
    return (GetGameTimer() - reloadSkinTimer) < 0
end
...
RegisterNetEvent('fivem-appearance:client:reloadSkin', function()
    if InCooldown() or CheckPlayerMeta() then
        QBCore.Functions.Notify("No puedes usar /fixpj ahora", "error")
        return
    end
        ...
        if isPointInside then
            ...
            if jobName == (clothingRoom.job or clothingRoom.gang) then
                if CheckDuty() then
                    inZone = true
                    local prefix = Config.UseRadialMenu and '' or '[E] '
                    QBCore.Functions.HelpNotify(prefix .. 'Clothing Room')
                end
            end
        else
            inZone = false
    ...
    playerOutfitRoomsCombo:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
            ...
            if isAllowed then
                inZone = true
                local prefix = Config.UseRadialMenu and '' or '[E] '
                QBCore.Functions.HelpNotify(prefix .. 'Outfits')
            end
        else
            inZone = false
            --xt()
...
CreateThread(function()
    while true do
        ...
        for k, v in pairs(Config.Tatto) do
            if #(ppos - v) < 6 then
                _s = 1
                QBCore.Functions.HelpNotify('<i class="fa-solid fa-paint-roller"></i> | ~INPUT_CONTEXT~ - Tatuajes.' )
                if IsControlJustReleased(0, 38) then
                    OpenTattooShop()
                end
            end
        end
        for k, v in pairs(Config.Barber) do
            if #(ppos - v) < 6 then
                _s = 1
                QBCore.Functions.HelpNotify('<i class="fa-duotone fa-mustache"></i> | ~INPUT_CONTEXT~ - Peluquería.' )
                if IsControlJustReleased(0, 38) then
                    OpenBarberShop()
                end
            end
        ...
        for k, v in pairs(Config.Clothes) do
            if #(ppos - v) < 6 then
                _s = 1
                QBCore.Functions.HelpNotify('<i class="fa-solid fa-clothes-hanger"></i> | ~INPUT_CONTEXT~ - Ropa.' )
                if IsControlJustReleased(0, 38) then
                    TriggerEvent('fivem-appearance:client:openClothingShopMenu')
                end
            end
...
CreateThread(function()
    for k, blipscoords in pairs(Config.Clothes) do
        local blip = AddBlipForCoord(blipscoords.x, blipscoords.y, blipscoords.z)
...
CreateThread(function()
    for k, blipscoords in pairs(Config.Barber) do
        local blip = AddBlipForCoord(blipscoords.x, blipscoords.y, blipscoords.z)
...
CreateThread(function()
    for k, blipscoords in pairs(Config.Tatto) do
        local blip = AddBlipForCoord(blipscoords.x, blipscoords.y, blipscoords.z)
        ...
        SetBlipColour(blip, 37)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString("Salón de tatuajes")
        EndTextCommandSetBlipName(blip)
    end
end)
